...
DB = os.getenv('DB_PATH', 'simulador.db')
ADMIN_PASS = os.getenv('ADMIN_PASS', 'jm.eng2025')
PRAZO = 420  # meses

# lê credenciais do ambiente (mais seguro) — se não estiverem setadas, usa os valores originais
...
# CSS global, logo e FontAwesome (coloque logo.jpeg em /static)
...
def send_email(nome, tel, renda, imovel, price, sac_ini, sac_fim, faixa):
    if SEND_EMAIL == '0':
        logging.info('Envio de email desativado por variável de ambiente SEND_EMAIL=0')
        return
...
def resultado(id):
    ...
    return STYLE + f"""
      ...
      <!-- botão WhatsApp -->
      <a href="https://api.whatsapp.com/send?phone=5538998721022&text=Ol%C3%A1,%20podemos%20conversar%20sobre%20minha%20futura%20casa?%20nome:%20{nome},%20Renda:%20{renda_txt},%20Im%C3%B3vel:%20{imovel_txt},%20Faixa:%20{faixa_txt},%201%C2%AA%20Parcela%20PRICE:%20R$%20{fmt(c[8])},%201%C2%AA%20Parcela%20SAC:%20R$%20{fmt(c[9])},%20%C3%9Altima%20SAC:%20R$%20{fmt(c[10])}" target="_blank" class="btn-custom btn-whatsapp w-100 mb-2">
...
def login():
    ...
    logo_url = url_for('static', filename='logo.jpeg')
    return STYLE + f"""
    <div class='box'>
      <img src='{logo_url}' class='logo'>
...
def get_dados():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, nome, valor FROM simulacao")  # Ajuste para o nome real da sua tabela e colunas
